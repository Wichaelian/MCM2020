...
#set initial demand at each site to average demand at that site
demands = np.array((df['Average Demand per Visit']))
#the amount that demand changes each day. 1/28 of average demand, weighted at
#each site depending on the proximity of nearby sites
increment = np.matmul(demands/28,dists)

excessdemand = 0 #cumulative unfulfilled demand
nvisits = np.zeros(70) #how often each site is visited - starts at 0

first = 0 #indexes for control testing
second = 1

for day in range(365):
    control = False #which formula to use. False for the good one
    ...
    if control:#same as in not control except we just visit every site in order
